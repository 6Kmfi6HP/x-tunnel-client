...
- Diagnostics page with one-click run-all checks, preset/custom direct/local-proxy HTTP tests that require successful HTTP status codes, local listen-port status badge, clearable direct/proxy network results, clearable forward route status chips, copyable diagnostics summary/report, copyable network and forward-test results, profile forward endpoint TCP tests, redacted export, and log access.
