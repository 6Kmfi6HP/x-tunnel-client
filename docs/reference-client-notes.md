...
- Added a Microsoft NCSI network-test preset using Microsoft's documented Windows connectivity probe URL (`http://www.msftconnecttest.com/connecttest.txt`).
...
- Added GUI smoke coverage that copies the connected network-test result and asserts both direct and HTTP proxy routes report HTTP 204.
...
- Extended the copied Settings folder summary with core status and version detail.
